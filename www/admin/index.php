...
require_once $_SERVER['DOCUMENT_ROOT'] . "/admin/version.php";
...
$xc['style'] = get_file('css', $xc['css'], $xc['update'], true, null);
$xc['scripts'] = get_file('js', $xc['js'], $xc['update'], true, null);
// ---------------------------------------------------------------------------------------------------------------------
...
if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/admin/modules/' . $xc['module'] . '/' . $xc['component'] . 'functions.php'))
    require_once $_SERVER['DOCUMENT_ROOT'] . '/admin/modules/' . $xc['module'] . '/' . $xc['component'] . 'functions.php';
...
$xc['categories'] = db_query("SELECT id, category AS name FROM db_startup_categories ORDER BY id ASC;");
...
if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/admin/modules/' . $xc['module'] . '/' . $xc['component'] . 'head.php'))
    $xc['head'] .= file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/admin/modules/' . $xc['module'] .'/' . $xc['component'] . 'head.php');
// ---------------------------------------------------------------------------------------------------------------------

// подключение главного шаблона

if (MOBILE == true && file_exists($_SERVER['DOCUMENT_ROOT'] . '/mob.inc.php'))
    require_once $_SERVER['DOCUMENT_ROOT'] . '/admin/mob.inc.php';

else
    require_once $_SERVER['DOCUMENT_ROOT'] . '/admin/tmp.inc.php';
// ---------------------------------------------------------------------------------------------------------------------
