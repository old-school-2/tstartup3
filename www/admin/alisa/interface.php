    ...
    switch ($protocol) {
        case 'alisa':
            ...
            foreach ($events as $e) {
                ...
                $answer_tts = $answer_tts . " " . $e['naming_command']  . ", ";
                    $description = mb_substr($e['short_description'], 0, 200);
                $list[] = array(
                    'image_id' => $e['img_alisa'],
                    'title' =>   ($i) . ". " . $e['naming_command']   ,
                    'description' => $description,
                    'button' => array('text' => 'стартап №' . $e['id']));
                if ($i == get_offset($protocol)){ break; }
            }

            $content =  array( 'text'  => $answer_text,
                                'tts'  => $answer_text . " " . $answer_tts,
                                "card" => array(
                                    'type' => 'ItemsList',
...
function get_startup_card($protocol, $data, $buttons, $e)
{
    $text = $e['naming_command'];
    switch ($protocol) {
